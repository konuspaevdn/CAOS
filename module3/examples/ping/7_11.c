...
uint16_t checksum(uint64_t len, void* buffer){
    int result = 0;
...
int main(int argc, char* argv[]) {
	...
	while((cur_time() - start_time) < timeout) {
		...
		uint16_t recv_checksum = recv.checksum;
		recv.checksum = 0;
		uint16_t  new_checksum = checksum(sizeof(request_t), &recv);
		if (new_checksum == recv_checksum) {
			++count;
		}
		printf("%d\n", recv.type);
		printf("%d\n", recv.code);
